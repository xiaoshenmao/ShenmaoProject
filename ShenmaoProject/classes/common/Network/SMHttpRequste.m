...
#import "SMHttpRequste.h"
#import <AFNetworking/AFNetworking.h>
#import "ApiConstansts.h"
#import <AFNetworking/AFNetworkActivityIndicatorManager.h>

@implementation SMHttpRequste
...
+ (void)postWithApi:(NSString *)api success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    AFHTTPSessionManager *manager = [SMHttpRequste AFHTTPSessionManager];
    [[AFNetworkActivityIndicatorManager sharedManager] setEnabled:YES];
    [manager POST:[SMHttpRequste urlWithApi:api] parameters:nil constructingBodyWithBlock:NULL progress:NULL success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (successBlock) {
...
+ (void)getWithURI:(NSString *)api success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    AFHTTPSessionManager *manager = [SMHttpRequste AFHTTPSessionManager];
    [[AFNetworkActivityIndicatorManager sharedManager] setEnabled:YES];
    [manager GET:[SMHttpRequste urlWithApi:api] parameters:nil progress:NULL success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (successBlock) {
            successBlock(responseObject);
        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
...
+ (NSString *)apiStringForReadDetailsWithReadType:(MLBReadType)readType {
    if (readType == MLBReadTypeSerial) {
        ...
        return [SMHttpRequste apiStringForReadWithReadType:readType];
    }
}
...
+ (void)requestPraiseCommentsWithType:(NSString *)type itemId:(NSString *)itemId firstItemId:(NSString *)firstItemId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiGetPraiseComments, type, itemId, firstItemId] success:successBlock fail:failBlock];
}
...
+ (void)requestTimeCommentsWithType:(NSString *)type itemId:(NSString *)itemId firstItemId:(NSString *)firstItemId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiGetTimeComments, type, itemId, firstItemId] success:successBlock fail:failBlock];
}
...
+ (void)requestReadDetailsWithType:(NSString *)type itemId:(NSString *)itemId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiGetReadDetails, type, itemId] success:successBlock fail:failBlock];
}
...
+ (void)requestRelatedsWithType:(NSString *)type itemId:(NSString *)itemId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiGetRelateds, type, itemId] success:successBlock fail:failBlock];
}
...
+ (void)requestHomeMoreWithSuccess:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:MLBApiHomePageMore success:successBlock fail:failBlock];
}
...
+ (void)requestReadCarouselWithSuccess:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:MLBApiReadingCarousel success:successBlock fail:failBlock];
}

+ (void)requestReadCarouselDetailsById:(NSString *)carouselId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:@"%@/%@", MLBApiReadingCarousel, carouselId] success:successBlock fail:failBlock];
}

// 文章列表
+ (void)requestReadIndexWithSuccess:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:MLBApiReadingIndex success:successBlock fail:failBlock];
}

// 短篇文章详情
+ (void)requestEssayDetailsById:(NSString *)essayId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiEssayDetailsById, essayId] success:successBlock fail:failBlock];
}

// 短篇文章评论列表
+ (void)requestEssayCommentsById:(NSString *)essayId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste requestPraiseCommentsWithType:MLBApiEssay itemId:essayId firstItemId:0 success:successBlock fail:failBlock];
}

// 短篇文章相关列表
+ (void)requestEssayRelatedsById:(NSString *)essayId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiGetRelateds, MLBApiEssay, essayId] success:successBlock fail:failBlock];
}

// 连载文章列表
+ (void)requestSerialListById:(NSString *)serialId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiSerialList, serialId] success:successBlock fail:failBlock];
}

#pragma mark - Music

+ (void)requestMusicIdListWithSuccess:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:MLBApiMusicIdList success:successBlock fail:failBlock];
}

// 音乐详情
+ (void)requestMusicDetailsById:(NSString *)musicId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiMusicDetailsById, musicId] success:successBlock fail:failBlock];
}

// 音乐详情评论点赞数降序排序列表
+ (void)requestMusicDetailsPraiseCommentsById:(NSString *)musicId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste requestPraiseCommentsWithType:MLBApiMusic itemId:musicId firstItemId:0 success:successBlock fail:failBlock];
}

// 音乐详情相似歌曲列表
+ (void)requestMusicDetailsRelatedMusicsById:(NSString *)musicId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiGetRelateds, MLBApiMusic, musicId] success:successBlock fail:failBlock];
}
...
+ (void)requestMovieListWithOffer:(NSInteger)offset success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiMovieList, offset] success:successBlock fail:failBlock];
}

// 获取电影详情
+ (void)requestMovieDetailsById:(NSString *)movieId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiMovieDetails, movieId] success:successBlock fail:failBlock];
}

// 获取电影故事列表
+ (void)requestMovieDetailsMovieStoriesById:(NSString *)movieId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiMovieStories, movieId, @"1", @"0"] success:successBlock fail:failBlock];
}
...
+ (void)requestMovieStoriesById:(NSString *)movieId firstItemId:(NSString *)firstItemId forDetails:(BOOL)forDetails success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiMovieStories, movieId, forDetails ? @"1" : @"0", firstItemId] success:successBlock fail:failBlock];
}

// 获取电影短评列表
+ (void)requestMovieDetailsMovieReviewsById:(NSString *)movieId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiMovieReviews, movieId, @"1", @"0"] success:successBlock fail:failBlock];
}
...
+ (void)requestMovieReviewsById:(NSString *)movieId firstItemId:(NSString *)firstItemId forDetails:(BOOL)forDetails success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiMovieReviews, movieId, forDetails ? @"1" : @"0", firstItemId] success:successBlock fail:failBlock];
}

// 获取电影评论列表
+ (void)requestMovieDetailsPraiseCommentsById:(NSString *)movieId success:(SuccessBlock)successBlock fail:(FailBlock)failBlock {
    [SMHttpRequste getWithURI:[NSString stringWithFormat:MLBApiGetPraiseComments, MLBApiMovie, movieId, @"0"] success:successBlock fail:failBlock];
}
